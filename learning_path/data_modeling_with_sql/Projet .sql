...
-- The resulting table should include the following columns: Employee Number, First Name and Last Name, Age, and Seniority.
    ...
    select  EMPLOYEE_int as NumberEmployee  , FIRST_NAME AS NAME , LAST_NAME as LastName , 
    SALARY , COMMISSION , (Salary + COALESCE(Commission, 0)) AS NetSalary ,
    DATEDIFF(year, BIRTH_DATE, GETDATE()) AS AGE -- calculate the age .
    from EMPLOYEES 
    where TITLE='Mr.' and (Salary + COALESCE(Commission, 0)) >=8000 -- COALESCE: main role is replace null into 0 . 
...
-- Other soluution 
    SELECT PRODUCTS.PRODUCT_REF, PRODUCTS.PRODUCT_NAME, PRODUCTS.SUPPLIER_int, PRODUCTS.UNITS_ON_ORDER, PRODUCTS.UNIT_PRICE
    FROM PRODUCTS
    WHERE (PRODUCTS.QUANTITY LIKE '%bottle%')AND (SUBSTRING(PRODUCT_NAME, 3, 1) = 'a' 
            OR SUBSTRING(PRODUCT_NAME, 3, 1) = 'A') AND (PRODUCTS.SUPPLIER_int IN (1, 2 ,3)) AND (PRODUCTS.UNIT_PRICE BETWEEN 70 AND  200)
            AND (PRODUCTS.UNITS_ON_ORDER IS NOT NULL);


--3/ Display customers who reside in the same region as supplier 1,
...
-- The resulting table should include all columns from the customer table.
    ...
    WHERE EXISTS (
            ...
            WHERE
                ...
                AND S.City = C.City
                AND RIGHT(S.POSTAL_CODE,3) = RIGHT(C.POSTAL_CODE , 3) -- what does it mean right ? begin from the right and he take the last three digits of the postal code 
        );
...
--The resulting table should display the columns: order number, new discount rate, and discount rate application note.
    ...
    od.ORDER_int , 
        ...
        CASE -- comme si if then , si alors 
    ...
    group by   (od.ORDER_int); --  we don't use group by in the aggregation function 
...
-- The resulting table should display the columns: supplier number, company, address, and phone number.
    select S.SUPPLIER_int , COMPANY , ADDRESS , PHONE
    from SUPPLIERS AS S  
    join PRODUCTS AS P 
    ON S.SUPPLIER_int = P.SUPPLIER_int 

    join CATEGORIES AS G 
    ON G.CATEGORY_CODE = P.CATEGORY_CODE 
    WHERE G.CATEGORY_NAME ='Beverages' 
    group by S.SUPPLIER_int , COMPANY , ADDRESS , PHONE -- we use group by beacause we have in the question "for each" 
    ORDER BY SUPPLIER_int; 
...
-- The resulting table should display the column: customer code.
    ...
    SELECT
        C.Customer_CODE AS CustomerCode , C.CITY , OD.QUANTITY , CAT.CATEGORY_NAME 
    FROM
        Customers AS C
    JOIN
        Orders AS O 
    ...
    WHERE
        C.City = 'Berlin'   and OD.QUANTITY >=1  and CAT.CATEGORY_NAME='DryPRODUCTS'; 


--7 Display customers who reside in France 
--and the total amount of orders they placed every Monday in April 1998 (considering customers who haven't placed any orders yet). 
 --The resulting table should display the columns: customer number, company name, phone number, total amount, and country.
    select C.CUSTOMER_CODE  , C.COMPANY , C.PHONE , SUM (SHIPPING_COST) AS TOTALAMOUNT , C.COUNTRY
    from     Customers AS C
    join Orders AS O
    ON C.CUSTOMER_CODE = O.CUSTOMER_CODE
    WHERE
    C.Country = 'France'
    AND O.ORDER_DATE BETWEEN '1998-04-01' AND '1998-04-30'-- IN APRIL 
    GROUP BY C.CUSTOMER_CODE  , C.COMPANY , C.PHONE , C.COUNTRY ; 
        
	 
--8 /Display customers who have ordered all products.
-- The resulting table should display the columns: customer code, company name, and telephone number.

    select C.CUSTOMER_CODE , COMPANY , PHONE 
    FROM CUSTOMERS AS C 
    WHERE NOT EXISTS ( --ensures the customer is excluded only if there exists a product they haven't  ordred . 
    SELECT p.PRODUCT_REF
    FROM PRODUCTS p
                WHERE NOT EXISTS ( -- cheks if the pproduct was ordred by the customer 
            SELECT 1 -- if true : exist or not exist  
            FROM ORDERS as o
...
--10/ Display the number of orders placed in 1996,
-- the number of orders placed in 1997, and the difference between these two numbers.
